...
def main():
    """Create enhanced visualizations with more details for presentation (English only)."""
    print("Generating sales data...")
    df = generate_synthetic_sales_data()
    
    # Create necessary directories
    os.makedirs('data/raw', exist_ok=True)
    os.makedirs('visualizations/english', exist_ok=True)
    ...
    print("Creating enhanced visualizations (English)...")
    ...
    bars = sns.barplot(x=day_names, y=day_sales.values, palette='Blues_d')
    ...
    plt.title('Average Daily Sales by Day of Week', fontweight='bold')
    ...
    bars = sns.barplot(x=month_names, y=month_sales.values, palette='Greens_d')
    ...
    if len(daily_sales) >= 14:  # Need enough data points
        ...
        plt.plot(trend.index, trend, label='Trend', linewidth=2, color='blue')
        plt.plot(seasonal.index, seasonal + trend.mean(), label='Seasonal Pattern', 
                 linewidth=1.5, color='green', alpha=0.7)
        
        plt.title('Sales Time Series Decomposition', fontweight='bold')
        plt.xlabel('Date')
        plt.ylabel('Units Sold')
        plt.legend(loc='best')
    ...
    plt.tight_layout()
    plt.savefig('visualizations/english/seasonal_patterns.png', bbox_inches='tight')
    plt.close()
    ...
    plt.subplot(2, 2, 1)
    plt.scatter(df['unit_price'], df['units_sold'], alpha=0.5, c=df['promotion'].astype('category').cat.codes, cmap='viridis')
    ...
    low_price = df[df['unit_price'] < avg_price]['units_sold'].mean()
    high_price = df[df['unit_price'] >= avg_price]['units_sold'].mean()
    
    price_elasticity = ((high_price - low_price) / avg_units) / ((avg_price * 1.1 - avg_price) / avg_price)
    
    plt.title('Price Elasticity Analysis', fontweight='bold')
    plt.xlabel('Price')
    plt.ylabel('Units Sold')
    plt.colorbar(label='Promotion Level')
    plt.grid(True, alpha=0.3)
    
    # Add elasticity calculation
    plt.annotate(f'Price Elasticity: {price_elasticity:.2f}', 
                xy=(df['unit_price'].max() * 0.7, df['units_sold'].max() * 0.8),
    ...
    plt.title('Average Sales by Promotion Level', fontweight='bold')
    ...
    plt.subplot(2, 1, 2)
    violin = sns.violinplot(x='product', y='unit_price', data=df, palette='Set3')
    
    # Rotate x labels for better visibility
    plt.xticks(rotation=45, ha='right')
    
    plt.title('Price Distribution by Product', fontweight='bold')
    plt.xlabel('Product')
    plt.ylabel('Price')
    plt.grid(axis='y', alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('visualizations/english/price_promotion_analysis.png', bbox_inches='tight')
    plt.close()
    ...
    plt.title('Total Sales by Product and Region', fontweight='bold')
    plt.xlabel('Region')
    plt.ylabel('Product')
    ...
    plt.title('Normalized Performance (0-1)', fontweight='bold')
    plt.xlabel('Region')
    plt.ylabel('Product')
    
    plt.tight_layout()
    plt.savefig('visualizations/english/product_region_heatmap.png', bbox_inches='tight')
    plt.close()
    ...
    plt.plot(daily_sales['date'], daily_sales['units_sold'], label='Historical Data', 
    ...
    plt.title('Sales Forecast for Next 30 Days with Confidence Bands', fontweight='bold', fontsize=18)
    ...
    plt.tight_layout()
    plt.savefig('visualizations/english/detailed_forecast.png', bbox_inches='tight')
    plt.close()
    ...
    for i, (metric, values) in enumerate(metrics.items()):
        ...
        plt.title(f'{metric}', fontweight='bold')
        
        # Add "Lower is better" note for error metrics
        if metric in ['MAE', 'RMSE', 'MAPE', 'Training Time (s)']:
            plt.annotate('Lower is better', xy=(0.5, 0.9), 
                         xycoords='axes fraction', ha='center', fontsize=8)
        else:
            plt.annotate('Higher is better', xy=(0.5, 0.9), 
                         xycoords='axes fraction', ha='center', fontsize=8)
    
    plt.tight_layout()
    plt.savefig('visualizations/english/advanced_model_comparison.png', bbox_inches='tight')
    plt.close()
    ...
    if len(years) > 1:
        ...
        plt.title('Monthly Sales Year-over-Year Comparison', fontweight='bold')
        plt.xlabel('Month')
        plt.ylabel('Units Sold')
        ...
        plt.title('Regional Sales Year-over-Year Comparison', fontweight='bold')
        plt.xlabel('Region')
        plt.ylabel('Units Sold')
        plt.legend(title='Year')
        plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('visualizations/english/year_over_year.png', bbox_inches='tight')
    plt.close()
    print("Year-over-year comparison saved")
    
    print("\nEnglish visualization creation completed!")
    print("- Visualizations saved to: visualizations/english/")
